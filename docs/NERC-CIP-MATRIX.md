...
| CIP-003 security management controls (technical governance evidence) | REQ-CIP-02 (this matrix), REQ-CIP-01 dependency mapping, and REQ-DEPLOY-01/02/04 approved critical region/resource-type/VM-size guardrails | Shared | Approved control mapping, control owner list, deployment-guardrail compliance, and review records | `docs/NERC-CIP-MATRIX.md`; Azure Policy compliance export; customer GRC/control library | Customer compliance owner | Review matrix and owner assignments; export deployment-guardrail compliance; store signed review evidence | Quarterly | Governance approvals and compliance program operation are outside Azure Policy |
| CIP-004 personnel & access governance signals and CIP-005 interactive remote access governance | REQ-ID-01/02 MFA baselines, REQ-ID-04 PIM-ready time-bound privileged access, REQ-ID-05 privileged-access inventory and Owner-count review, REQ-ID-06 CIEM findings | Shared | Conditional Access/PIM configuration exports, privileged-access review reports, CIEM findings disposition | `docs/ENTRA-CONDITIONAL-ACCESS-PIM.md`; `docs/AZURE-RBAC-PIM.md`; `docs/ACCESS-REVIEWS.md`; customer identity-governance evidence store | Identity security owner | Export Entra configuration + run scripted access reviews + collect CIEM outputs and decision records | Quarterly and after privileged-access incidents | Personnel risk assessment, training, onboarding/termination workflow execution, and final access-certification approvals remain customer-owned processes |
...
| CIP-005 interactive remote access monitoring telemetry | REQ-LOG-01 Activity Log export control (composed in REQ-CIP-01 overlay), REQ-DEF-07/08 Windows/Linux AMA readiness (composed), and broader v2 REQ-LOG-02 resource diagnostics coverage (not an overlay member) | Shared | Activity Log export assignment/compliance, AMA readiness findings, diagnostics assignment/compliance records | Azure Policy compliance export; Log Analytics/Sentinel retention records | SOC/platform owner | Validate Activity Log export and AMA readiness in overlay scope; reconcile broader diagnostics coverage separately | Monthly | Session monitoring procedures, alert triage decisions, and investigator response actions remain customer process controls |
...
| CIP-008 incident detection/response technical telemetry | REQ-LOG-01 Activity Log export (overlay member), broader REQ-LOG-02 diagnostics coverage (not an overlay member), REQ-DEF-06 vulnerability-assessment signal, and optional Defender plan signals from REQ-DEF-02/03/04 | Shared | Activity/resource logging assignment-compliance evidence plus Defender findings/signals exports | Azure Policy compliance export; Defender findings exports; SOC runbooks and case records | SOC owner | Validate Activity Log and diagnostics telemetry routing; collect Defender findings outputs and incident decision records | Monthly for control, per incident for process evidence | Microsoft Sentinel onboarding/analytics/incident workflow evidence, response command timelines, external notifications, and exercise evidence are customer-owned manual controls outside this repository |
...
| CIP-006/CIP-013 adjacent dependencies | No direct v2 technical automation beyond supporting signals above | Customer | N/A from Azure Policy | Customer physical-security and supply-chain systems | Customer compliance owner | Maintain separate procedural evidence and cross-reference this matrix | Per internal policy | Physical access governance and supply-chain risk-management programs remain outside repository automation |
...
- NERC service-principal/access-review governance dependency issue: https://github.com/johnstel/azureeslzmultisubdemo/issues/21
...
- Microsoft Azure compliance offering for NERC: https://learn.microsoft.com/en-us/azure/compliance/offerings/offering-nerc
- Microsoft NERC cloud/shared-responsibility guidance: https://learn.microsoft.com/en-us/azure/compliance/offerings/offering-nerc#shared-responsibility-in-the-cloud
...
- NERC CIP standards landing page: https://www.nerc.com/standards/reliability-standards/cip
