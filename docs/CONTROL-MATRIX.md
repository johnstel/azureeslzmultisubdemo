...
**Bicep implementation (main.bicep):** REQ-DEF-02/03/04 are wired via `modules/defender-plan-assignment.bicep`, pinned to the verified built-in `definitionId`/`definitionVersion` (`1.*.*`) for each plan with no free-text override. There is no opt-in parameter: `effect` is hardcoded to `Disabled` and the assignment never creates a managed identity, so a normal deployment can never enable a paid plan or create standing Owner access. These three built-ins only support remediation via the Owner role at the subscription scope, and this repository's automatic RBAC granting (`modules/remediating-policy-assignment.bicep`) intentionally refuses to grant Owner or User Access Administrator to any managed identity (privilege-escalation guardrail); a single management-group-scoped identity inherited across every descendant subscription would be Owner everywhere at once, which is too broad a blast radius to grant automatically without a separately approved, time-bounded workflow that is out of scope for this demo template — hence `manual-evidence`. Enabling any of these plans is a fully independent action a customer takes directly against Microsoft Defender for Cloud (Azure Portal or `az security pricing create`), including reviewing current licensing/cost and any role assignments Microsoft's own tooling then requires; whether that action provisions agentless scanning or the Azure Monitor Agent for Defender for Servers depends on the sub-plan/settings chosen at that time and is not configured or claimed by this template. REQ-DEF-01 (the unsafe, all-or-nothing initiative) and REQ-DEF-05 (the deprecated MMA auto-provisioning policy) are intentionally never assigned. REQ-DEF-06 is unconditionally assigned (no parameter) via `modules/policy-assignment.bicep`, also pinned to `definitionVersion: '3.*.*'`, because it is a free, audit-only signal. None of these controls prove Microsoft Cloud Security Benchmark (MCSB, REQ-BASE-01) or regulatory-compliance-dashboard compliance on their own; they populate Defender for Cloud recommendations and CIEM findings that a customer must still review.
