...
- **Management-port and NSG audits:** REQ-NET-01 and REQ-NET-03 rely on Microsoft Defender for Cloud security assessments populated by the Microsoft Defender CSPM plan (REQ-DEF-02), which they now explicitly depend on rather than the unsafe, all-or-nothing REQ-DEF-01 initiative. Enabling that specific plan (with its `effect` parameter explicitly overridden to `Disabled` by default per this repository's no-cost profile, and only opted in to `DeployIfNotExists` deliberately) populates compliance data for these audits; it does not require a second, separate custom rule.
