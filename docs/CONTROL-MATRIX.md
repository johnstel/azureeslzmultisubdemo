...
- MCSB (REQ-BASE-01) and CIS (REQ-BASE-02) were audited member-by-member for remediation-capable (DeployIfNotExists/Modify) policies; none were found, so both set `remediationIdentityRequired: false` and require no `roleDefinitionIds`. NIST SP 800-53 Rev. 5 (REQ-BASE-03) does contain a small number of remediation-capable members and requires the Contributor role (`b24988ac-6180-42a0-ab88-20f7382dd24c`); a separate, newer "NIST SP 800-53 R5.1.1" initiative also exists (GUID `60205a79-6280-4e20-a147-e2011e09dc78`) for the updated revision. See each record's `notes` field for the full verification method and its limits.
