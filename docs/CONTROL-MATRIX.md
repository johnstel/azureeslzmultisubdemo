...
**REQ-NET-01/02 issue #13 implementation:** the workload initiative defaults to `Audit` and its assignment defaults to `DoNotEnforce`. Public management ingress recognizes `*`, `Internet`, and IPv4 `0.0.0.0/0` in singular and array source aliases, and detects TCP/any-protocol rules containing destination `22`, `3389`, or `*`. Approved private management paths remain in scope because they do not use an any-public source. Exceptional public access and special-purpose workload subnets require a documented, time-bound Azure Policy exemption; Platform and Connectivity are not assigned this initiative. The existing demo-root public-IP audit remains the single hierarchy-wide public-IP control.
