...
**Bicep implementation (main.bicep):** REQ-DEF-02/03/04 are wired via `modules/defender-plan-assignment.bicep` and the `enableDefenderCspm`, `enableDefenderForServers`, and `enableDefenderForStorage` parameters (each default `false`). Each assignment always carries a `SystemAssigned` identity because Azure Policy requires an identity on any DeployIfNotExists-capable assignment, but the template never grants that identity a role: REQ-DEF-02/03/04 all require the built-in Owner role for remediation, and this repository's automatic RBAC granting (`modules/remediating-policy-assignment.bicep`) intentionally refuses to grant Owner or User Access Administrator to any managed identity (privilege-escalation guardrail). A subscription Owner must manually grant the Owner role to the assignment's `identityPrincipalId` output before setting the corresponding parameter to `true`; until then the assignment safely stays at `effect: Disabled` and does nothing. Enabling any of these plans also requires reviewing current Microsoft Defender for Cloud licensing/cost, since they are billed per-resource. REQ-DEF-01 (the unsafe, all-or-nothing initiative) and REQ-DEF-05 (the deprecated MMA auto-provisioning policy) are intentionally never assigned. REQ-DEF-06 is unconditionally assigned (no parameter) via `modules/policy-assignment.bicep` because it is a free, audit-only signal. None of these controls prove Microsoft Cloud Security Benchmark (MCSB, REQ-BASE-01) or regulatory-compliance-dashboard compliance on their own; they populate Defender for Cloud recommendations and CIEM findings that a customer must still review.
