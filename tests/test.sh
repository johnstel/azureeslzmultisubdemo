...
printf '1/15 Validate repository versioning and branch guidance...\n'
version_value="$(tr -d '\r\n' < "${PROJECT_DIR}/VERSION")"
...
printf '2/15 Build the complete tenant template...\n'
...
printf '3/15 Validate the ARM parameter template...\n'
jq -e '
...
printf '4/15 Confirm there are exactly two subscription associations...\n'
...
printf '5/15 Confirm no paid always-on resource types are declared outside the opt-in central monitoring module...\n'
if rg -n \
...
printf '6/15 Confirm tenant-root scope is only used as the parent hierarchy input...\n'
...
printf '7/15 Confirm five distinct Entra group parameters and guarded scripts...\n'
...
printf '8/15 Confirm region policy safely permits global resources...\n'
...
printf '9/15 Confirm central monitoring defaults create no metered resources...\n'
jq -e '
...
printf "10/15 Confirm central monitoring guards against conflicting new/existing workspace inputs and Sentinel-without-workspace...\n"
...
printf '11/15 Confirm the central monitoring module exposes an effective workspace ID output...\n'
...
printf '12/15 Confirm invalid central monitoring configurations fail deployment explicitly...\n'
...
printf '13/15 Confirm teardown scripts protect a supplied existing workspace resource group and only remove a demo-created monitoring resource group...\n'
...
printf '14/15 Confirm a whitespace-only existing workspace resource ID never triggers deletion of the monitoring resource group...\n'
...
printf '15/15 Parse cross-platform scripts and check macOS Bash 3.2 compatibility...\n'
