...
printf '1/19 Validate repository versioning and branch guidance...\n'
version_value="$(tr -d '\r\n' < "${PROJECT_DIR}/VERSION")"
...
printf '2/19 Build the complete tenant template...\n'
...
printf '3/19 Validate the ARM parameter template...\n'
jq -e '
...
printf '4/19 Confirm there are exactly two unconditional subscription associations...\n'
...
printf '5/19 Confirm no paid always-on resource types are declared outside the opt-in central monitoring module...\n'
if rg -n \
...
printf '6/19 Confirm tenant-root scope is only used as the parent hierarchy input...\n'
...
printf '7/19 Confirm five distinct Entra group parameters and guarded scripts...\n'
...
printf '8/19 Confirm region policy safely permits global resources...\n'
...
printf '9/19 Confirm the Critical Infrastructure branch is opt-in and correctly wired...\n'
...
printf '10/19 Confirm criticalInfrastructureSubscriptionIds validates duplicates and overlap...\n'
...
printf '11/19 Confirm teardown scripts move critical subscriptions and delete the Critical Infrastructure management group before Landing Zones...\n'
...
printf '12/19 Confirm central monitoring defaults create no metered resources...\n'
jq -e '
...
printf "13/19 Confirm central monitoring guards against conflicting new/existing workspace inputs and Sentinel-without-workspace...\n"
...
printf '14/19 Confirm the central monitoring module exposes an effective workspace ID output...\n'
...
printf '15/19 Confirm invalid central monitoring configurations fail deployment explicitly...\n'
...
printf '16/19 Confirm teardown scripts protect a supplied existing workspace resource group and only remove a demo-created monitoring resource group...\n'
...
printf '17/19 Confirm a whitespace-only existing workspace resource ID never triggers deletion of the monitoring resource group...\n'
...
printf '18/19 Parse cross-platform scripts and check macOS Bash 3.2 compatibility...\n'
...
printf '19/19 Validate the v2 control catalog (schema-equivalent checks + matrix consistency)...\n'
