...
printf '5/27 Confirm no paid always-on resource types are declared outside the opt-in central monitoring and backup vault modules...\n'
...
printf '27/27 Confirm backup coverage and vault posture controls stay audit-first and dependency-gated...\n'
jq -e --slurpfile catalog "${control_catalog}" '
  def deployment($name):
    first(.. | objects | select(.type? == "Microsoft.Resources/deployments" and .name? == $name));
  def control($id):
